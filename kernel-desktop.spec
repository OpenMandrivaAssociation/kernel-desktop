...
Source:        kernel-desktop-3.1.2-1.tar.bz2
...
%prep
%setup -q -n kernel-desktop-3.1.2-1

%build
make defconfig
make -s kernelrelease
test $(make -s kernelrelease) = 3.1.2-1.1-desktop
make %{?_smp_mflags}
...
cp %{bzImage} %{buildroot}/boot/vmlinuz-3.1.2-1.1-desktop
cp System.map %{buildroot}/boot/System.map-3.1.2-1.1-desktop
cp .config    %{buildroot}/boot/config-3.1.2-1.1-desktop
ln -snf /usr/src/devel/3.1.2-1.1-desktop %{buildroot}/lib/modules/3.1.2-1.1-desktop/build
ln -snf build %{buildroot}/lib/modules/3.1.2-1.1-desktop/source

mkdir -p %{buildroot}/usr/src/devel/3.1.2-1.1-desktop
cat develfiles-%asmarch.list >>develfiles.list
rsync -ar --files-from=develfiles.list . %{buildroot}/usr/src/devel/3.1.2-1.1-desktop

%post -n kernel-desktop
/sbin/installkernel %{instkern_opts} 3.1.2-1.1-desktop

%preun -n kernel-desktop
/sbin/installkernel -R 3.1.2-1.1-desktop

%postun -n kernel-desktop
/sbin/kernel_remove_initrd 3.1.2-1.1-desktop
...
%defattr (-, root, root)
%dir /lib/modules
/lib/modules/3.1.2-1.1-desktop
/boot

%files -n kernel-desktop-devel
%defattr (-, root, root)
/usr/src/devel/3.1.2-1.1-desktop

%files -n kernel-desktop-debuginfo -f debugfiles.list
%defattr (-, root, root)

%changelog
* Tue Nov 22 2011 Franck Bui <franck.bui@mandriva.com> 3.1.2-1.1-desktop
  + Mandriva Release v3.1.2-1
